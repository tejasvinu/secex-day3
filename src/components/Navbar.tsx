        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {isAdmin && (
            <>
              ...
              {/* Add link to Successful Events page */}
          ...
          <Sheet>
            ...
            <SheetContent side="right">
              <nav className="grid gap-4 py-6">
                ...
                {isAdmin && (
                  <>
                    <Link href="/admin/dashboard" className="flex items-center gap-2"><ShieldCheck size={18}/> Admin Dashboard</Link>
                    <Link href="/admin/observations" className="flex items-center gap-2"><FileText size={18}/> Verify Events</Link>
                    {/* Add link to Successful Events page */}
                    <Link href="/admin/successful-events" className="flex items-center gap-2"><Trophy size={18}/> Successful Events</Link>
                  </>
                )}
                ...
                {status === 'unauthenticated' && (
                  <>
                    ...
                    {/* <Link href="/register" className="flex items-center gap-2"><UserPlus size={18}/> Register</Link> */}
                  </>
                )}

                {status === 'authenticated' && (
                  <Button variant="ghost" onClick={() => signOut({ callbackUrl: '/' })} className="flex items-center gap-2 justify-start w-full text-left">
